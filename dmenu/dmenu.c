...
drawhighlights(struct item *item, int x, int y, int maxw)
{
	...
	drw_setscheme(drw, scheme[item == sel ? SchemeSelHighlight : item->out ? SchemeOut : SchemeNormHighlight]);
	strcpy(tokens, text);
	for (token = strtok(tokens, " "); token; token = strtok(NULL, " ")) {
		...
		while (highlight) {
			...
			if (indentx - (lrpad / 2) < maxw)
				drw_text(
					drw, x + indentx - (lrpad / 2), y,
					MIN(maxw - indentx,
...
usage(void)
{
	die("usage: dmenu [-bcfiv] [-l lines] [-p prompt] [-fn font] [-m monitor] [-hp items]\n"
	    "             [-nb color] [-nf color] [-sb color] [-sf color] [-hf color] [-w windowid]", stderr);
}
